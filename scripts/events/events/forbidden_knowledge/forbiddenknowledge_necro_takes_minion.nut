this.forbiddenknowledge_necro_takes_minion <- this.inherit("scripts/events/event", {
	m = {
		LastCombatID = 0,
		ChosenBackground = "",
		Dude = null,
	...
	function standardStart(_event, special){
		...
		if (_event.m.ChosenBackground == "" || _event.m.ChosenBackground ==  null) {
			::logInfo("DNECRO: Warning - Background is blank! Replacing with vagabond background.")
		...
		local standardOptions = [
			{
				...
				function getResult( _event )
				{
					...
					_event.m.Dude.worsenMood(2.0, "Has become the minion of a necromancer.");
					_event.m.Dude = null;
					return 0;
				}
